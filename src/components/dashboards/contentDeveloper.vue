<template>
  <div>
    <v-app-bar dense color="blue white--text darken-1" dark>
      <v-app-bar-nav-icon v-on:click="side_bar = !side_bar">
        <v-icon>dehaze</v-icon>
      </v-app-bar-nav-icon>
      <v-toolbar-title style="width: 500px" class="ml-0 pl-4">
        <span class="hidden-sm-and-down">Content Developer Dashboard</span>
      </v-toolbar-title>
      ...
      <v-menu bottom :offset-y="offset" :nudge-width="200">
        ...
        <v-list>
          <v-list-item v-for="(item, index) in vmenuitem" :key="index" router :to="item.link">
            <v-list-item-title>{{ item.title }}</v-list-item-title>
          </v-list-item>
          <v-btn class="mx-5" icon large @click="logout">Logout</v-btn>
        </v-list>
      </v-menu>
    </v-app-bar>
    ...
    return {
      ...
      items: [
        { title: "Dashboard", icon: "mdi-gauge-full", link: "/reb-admin/" },
        {
          ...
          link: "/reb-admin/profile/",
        },
        { title: "Courses", icon: "book", link: "/reb-cd/courses/" },
        { title: "Course", icon: "book", link: "/reb-cd/course/" },
      ],
      vmenuitem: [
